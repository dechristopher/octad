...
func squaresAreAttacked(pos *Position, sqs ...Square) bool {
	...
	for _, sq := range sqs {
		...
		if pos.Turn() == White {
			capRight := (pos.board.bbBlackPawn & ^bbFileD & ^bbRank1) << 7
			...
			capRight := (pos.board.bbWhitePawn & ^bbFileD & ^bbRank4) >> 9
			capLeft := (pos.board.bbWhitePawn & ^bbFileA & ^bbRank4) >> 7
			bb = (capRight | capLeft) & bbForSquare(sq)
			if bb != 0 {
				return true
			}
...
var (
	bbFiles = [8]bitboard{bbFileA, bbFileB, bbFileC, bbFileD}
	bbRanks = [8]bitboard{bbRank1, bbRank2, bbRank3, bbRank4}
